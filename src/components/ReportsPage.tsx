...
const ReportsPage: React.FC = () => {
  ...
  const fetchAllData = async () => {
    setIsLoading(true);
    try {
      const [attendanceRes, studentsRes, teachersRes, categoriesRes] =
        await Promise.all([
  ...
  const generateReportData = (
    ...
    if (selectedDateRange.endDate) {
      filteredAttendance = filteredAttendance.filter(
        (record) => new Date(record.date) <= new Date(selectedDateRange.endDate)
      );
    }

    if (selectedForm) {
      filteredAttendance = filteredAttendance.filter(
        (record) => record.form === selectedForm
      );
    }
    ...
    last7Days.forEach((date) => {
      ...
      dailyStats[date] = dayRecords.filter(
        (r) => r.status === "present"
      ).length;
    });
    ...
    categories.forEach((category) => {
      const formRecords = filteredAttendance.filter(
        (r) => r.form === category.name
      );
    ...
    const genderStats = {
      ...
      female: students.filter((s) => s.gender?.toLowerCase() === "female")
        .length,
    ...
    try {
      ...
      if (formChartRef.current) {
        ...
        if (ctx) {
          ...
          const presentData = formLabels.map(
            (form) => reportData.formStats[form].present
          );
          const absentData = formLabels.map(
            (form) => reportData.formStats[form].absent
          );
            ...
            <nav className="hidden lg:flex items-center space-x-8">
              ...
              <div className="flex items-center space-x-4">
                ...
                >
                  ...
                  <span> {formatMessage({ id: "AdminProfile.logout" })}</span>
                </button>
              </div>
            </nav>
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 shadow-lg">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
              <div className="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-button-green">
                    {" "}
                    {formatMessage({ id: "reports.filters.startDate" })}
                  </label>
                  <input
                    type="date"
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-button-green">
                    {" "}
                    {formatMessage({ id: "reports.filters.endDate" })}
                  </label>
                  <input
                    type="date"
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-button-green">
                    {" "}
                    {formatMessage({ id: "reports.filters.form" })}
                  </label>
                  <select
                    value={selectedForm}
                  ...
                  >
                    <option value="">
                      {" "}
                      {formatMessage({ id: "reports.filters.allForms" })}
                    </option>
              ...
              <div className="flex items-center space-x-4">
                ...
                >
                  ...
                  <span>
                    {" "}
                    {formatMessage({ id: "reports.filters.refresh" })}
                  </span>
                </button>
              </div>
            </div>
          ...
          >
            ...
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 shadow-lg">
              <h3 className="text-xl font-bold text-white mb-4 flex items-center space-x-2">
                <PieChart className="w-5 h-5 text-button-green" />
                <span>
                  {formatMessage({ id: "reports.charts.attendanceRate" })}
                </span>
              </h3>
            ...
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 shadow-lg">
              <h3 className="text-xl font-bold text-white mb-4 flex items-center space-x-2">
                <BarChart3 className="w-5 h-5 text-button-green" />
                <span>
                  {formatMessage({ id: "reports.charts.formStatistics" })}
                </span>
              </h3>
              <div className="h-64 relative">
                <canvas
            ...
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 shadow-lg md:col-span-2 lg:col-span-1">
              <h3 className="text-xl font-bold text-white mb-4 flex items-center space-x-2">
                <TrendingUp className="w-5 h-5 text-button-green" />
                <span>
                  {formatMessage({ id: "reports.charts.monthlyTrends" })}
                </span>
              </h3>
              <div className="h-64 relative">
                <canvas
